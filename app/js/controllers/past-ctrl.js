App.controller('PastController', function ($scope, $http, $location, $cookies, $cookieStore, MY_CONSTANT,$state, $timeout, $window, ngDialog) {
    'use strict';
    $scope.loading = true;

    var getPastShipmentList = function () {
        var accessToken=$cookieStore.get('accessToken');
        //var i=0;
        //var j=0;
        $http({
            url: MY_CONSTANT.url + '/api/v1/admin/getAllPastBookings',
            method: "GET",
            headers: {
           ...
           // $scope.nationalPermit = data.notionalPermit[0].npDoc;
            if (status == 200) {
                ...
                custList.forEach(function (column) {
                    ...
                    if(column.bookingStatus=="COMPLETED")
                        d.bookingStatus="Bid Closed";
                    else
                        d.bookingStatus=column.bookingStatus;
                    d.paymentMode=column.paymentMode;
                    d.promoCode="--";
                    d.consigmentNote= column.doc.consigmentNote?column.doc.consignmentNote:'app/img/no_image.png';
    ...
    $scope.zoom=function(data_get)
    {
        ...
        if(data_get.profilePicture != 'app/img/no_image.png' )
...
// ============================================================================
    $scope.exportExcel = function () {
        alasql('SELECT * INTO CSV("Past_Excel.csv",{headers:true}) FROM ?', [$scope.excelList]);
    };
});
